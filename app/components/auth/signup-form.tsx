...
const SignupForm = () => {
   ...
   *
   * Antes de enviar al servidor, validamos que:
   * 1. Las contraseñas coincidan
   * 2. La contraseña tenga al menos 6 caracteres
   *
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    // Validación en cliente para UX inmediata
  ...
  return (
    <form action={handleSubmit} className="space-y-5">
        ...
        {/* Note: Not adding 'name' field as server action might not support it yet, adhering to existing fields */}
        ...
        <div>
            ...
            <input 
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 text-white outline-none focus:ring-2 focus:ring-blue-600 transition-all placeholder:text-slate-700"
            />
            <p className="text-[10px] text-slate-600 mt-2 ml-1 italic">Mínimo 8 caracteres, incluye un símbolo.</p>
        </div>

        <div>
            <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-[0.15em] mb-2 ml-1">Confirmar Contraseña</label>
            <input 
              type="password" 
